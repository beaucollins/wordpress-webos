...
XMLRPCParser = function(string){
  ...
  var cleaned = XMLRPCParser.protectStrings(XMLRPCParser.cleanBOM(XMLRPCParser.cleanWhiteSpace(string)));
...
// prevents empty text nodes.
XMLRPCParser.cleanWhiteSpace = function(string){
...
XMLRPCParser.prototype.parse = function(node){
  ...
  switch(node.nodeName){
    ...
    case 'string':
      ...
      return s.replace(/&amp;([a-z]+|\#[\d]+);/ig, '&$1;').replace(/&gt;/gi,'>').replace(/&lt;/gi,'<');
      break;
